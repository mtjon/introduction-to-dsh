...
- Windows _usually_ does not, but there are some options: 
    ...
    - [Git for windows](https://gitforwindows.org/) allows you to run git commands, through a bash shell.
...
# Verifying you really don't have SSH
Under Windows, open Powershell (WIN+X, A, click `yes`).

If this gives feedback stating it does not exist, you do not have ssh, and should continue to `getting WSL`, or `getting Git Bash`. If it gives feedback stating it does not have enough parameters, you have ssh, and should continue to one of the `generating a keypair`-chapters.
